...
def parse_fit_file(file_content: bytes) -> Tuple[pd.DataFrame, int, datetime]:
    ...
    # **Handle missing timestamps gracefully (Improvement from suggestion)**
    ...
    # --- ✅ FIX APPLIED HERE ---
    # Create the new time column that Plotly can format correctly.
    df['time_hms'] = pd.to_timedelta(df['time'], unit='s')
    # --- END FIX ---
    ...
    if 'speed' in df.columns:
        ...
        # **Optional: Smooth speed data (Improvement from suggestion)**
        df['speed_kmh'] = df['speed_kmh'].rolling(3, min_periods=1).mean()
    # **Optional: Ensure distance is cumulative (Improvement from suggestion)**
...
if 'results' in st.session_state:
    ...
    with tabs[0]: # Summary
        ...
        with c1: st.metric("Total Distance", f"{metrics.get('total_distance', 'N/A')} km")
        with c2:
        ...
        with c3: st.metric("Average Speed", f"{metrics.get('avg_speed_overall', 'N/A')} km/h")
        c4, c5, c6 = st.columns(3)
        total_work_val, work_above_cp_val = metrics.get('total_work_kj'), metrics.get('total_work_above_cp_kj')
        total_work_per_kg_val, work_above_cp_per_kg_val = metrics.get('total_work_kj_per_kg'), metrics.get('total_work_above_cp_kj_per_kg')
        with c4:
    ...
    with tabs[1]: # Interval Analysis
        ...
        # --- NEW: Default Smoothing UI ---
        smooth_power = st.checkbox("Smooth Power Data", value=True, help="Apply a rolling average to the power data to reduce noise.")
        smoothing_window = 3
        if smooth_power:
            smoothing_window = st.slider("Smoothing Window (seconds)", 1, 30, 3, help="Number of seconds for the rolling average.")
        ...
        # --- ✅ FIX APPLIED HERE: Use 'time_hms' for the x-axis ---
        ...
        fig_wbal_percent.add_annotation(x=df['time_hms'].iloc[len(df)//2], y=5, text="Danger Zone", showarrow=False, font=dict(color="white", size=12), xanchor='center', yanchor='middle')
        ...
        zone_colors = {"Green (70-100%)": 'green', "Yellow (50-70%)": 'yellow', "Orange (30-50%)": 'orange', "Orange (10-30%)": 'darkorange', "Danger Zone (0-10%)": '#8B0000'}
        fig_wbal_zones = go.Figure(go.Bar(x=wbal_zones_df['Time (s)'], y=wbal_zones_df['Zone'], orientation='h', text=wbal_zones_df['Chart Text'], marker_color=[zone_colors.get(zone, 'grey') for zone in wbal_zones_df['Zone']]))
        ...
        with col2: max_duration = st.slider("Max duration of effort (seconds):", 1, 600, 300)
        with col3: recovery_tolerance = st.slider("Recovery Tolerance (s):", 1, 30, 5, help="Allowable recovery time within an effort before it's considered ended.")
        ...
        # --- ✅ FIX APPLIED HERE: Use 'time_hms' for the x-axis ---
        fig_depletion.add_trace(go.Scatter(x=df['time_hms'], y=df['wbal_kj'], name='W\'bal (kJ)', line=dict(color='#9467bd', width=2)), secondary_y=False)
    ...
    with tabs[2]: # Match Analysis
        ...
        fig_matches.add_trace(go.Scatter(x=matches_df['Duration (s)'], y=matches_df['Magnitude (%CP)'], mode='markers', name='Matches', marker=dict(color=matches_df["Depletion (% W')"], colorscale='RdYlGn_r', showscale=True, colorbar=dict(title="W' Depletion %"))))
        ...
        for i, depletion in enumerate(depletion_levels):
            ...
            fig_matches.add_trace(go.Scatter(x=durations, y=magnitude, mode='lines', line=dict(dash='dot', color=colors[i]), name=f'{depletion}% W\'', showlegend=False))
            fig_matches.add_annotation(x=durations[-1], y=magnitude[-1], text=f" {depletion}%", showarrow=False, xanchor='left', font=dict(color=colors[i]))
        fig_matches.update_layout(title_text='Match Magnitude vs. Duration', template='plotly_white', font=dict(color="black"), legend=dict(orientation="h", yanchor="bottom", y=-0.3, xanchor="center", x=0.5))
    ...
    with tabs[4]: # Power Profile
        ...
        for i in range(0, len(duration_keys), 2):
            with cols[i//2]:
                label1, duration1 = duration_keys[i], key_durations[duration_keys[i]]
                power_value1 = mmp_data_w.get(duration1)
                st.metric(label=label1, value=f"{int(power_value1)} W" if power_value1 is not None else "N/A")
                if i + 1 < len(duration_keys):
                    label2, duration2 = duration_keys[i+1], key_durations[duration_keys[i+1]]
                    power_value2 = mmp_data_w.get(duration2)
                    st.metric(label=label2, value=f"{int(power_value2)} W" if power_value2 is not None else "N/A")
        fig_mmp = go.Figure(go.Scatter(x=mmp_df['Duration (s)'], y=mmp_df['Max Power (W)'], mode='lines+markers'))
        fig_mmp.update_layout(title_text='Mean Maximal Power (MMP) Curve', template='plotly_white', font=dict(color="black"), xaxis_type="log", xaxis=dict(tickmode='array', tickvals=[1, 5, 10, 30, 60, 120, 300, 600, 1200, 1800, 3600], ticktext=['1s', '5s', '10s', '30s', '1m', '2m', '5m', '10m', '20m', '30m', '60m']))
        ...
        if weight > 0:
            ...
            for i in range(0, len(duration_keys), 2):
                with cols_wkg[i//2]:
                    ...
                    st.metric(label=label1, value=f"{power_value_wkg1:.2f} W/kg" if power_value_wkg1 is not None else "N/A")
                    if i + 1 < len(duration_keys):
                        label2, duration2 = duration_keys[i+1], key_durations[duration_keys[i+1]]
                        power_value_wkg2 = mmp_data_wkg.get(duration2)
                        st.metric(label=label2, value=f"{power_value_wkg2:.2f} W/kg" if power_value_wkg2 is not None else "N/A")
            fig_mmp_wkg = go.Figure(go.Scatter(x=mmp_df['Duration (s)'], y=mmp_df['Max Power (W/kg)'], mode='lines+markers', line=dict(color='orange')))
            fig_mmp_wkg.update_layout(title_text='Mean Maximal Power (MMP) Curve (W/kg)', template='plotly_white', font=dict(color="black"), xaxis_type="log", xaxis=dict(tickmode='array', tickvals=[1, 5, 10, 30, 60, 120, 300, 600, 1200, 1800, 3600], ticktext=['1s', '5s', '10s', '30s', '1m', '2m', '5m', '10m', '20m', '30m', '60m']))
    ...
    with tabs[5]: # Route Maps
        ...
        if 'position_lat' in df.columns and 'position_long' in df.columns and not df[['position_lat', 'position_long']].dropna().empty:
            ...
            ColorLine(positions=coordinates, colors=gps_df['Wbal_percent'], colormap=wbal_colormap, weight=5).add_to(m_wbal)
    ...
    with tabs[6]: # Data Explorer
        ...
        if selected_metrics:
            ...
            for metric in selected_metrics:
                ...
                # --- ✅ FIX APPLIED HERE: Use 'time_hms' for the x-axis ---
                fig_explorer.add_trace(go.Scatter(x=df['time_hms'], y=smoothed_data, name=metric), secondary_y=is_secondary)
                # --- END FIX ---
    ...
    with tabs[8]: # Beta Features
        ...
        if not df.empty and df['time'].max() > 0:
            ...
            with pacing_cols[0]:
                ...
                else: st.write("No data in the first half.")
            with pacing_cols[1]:
                ...
                else: st.write("No data in the second half.")
        else: st.info("Not enough data for pacing analysis.")
        st.divider()
        ...
        if 'df2' in results:
            ...
            # --- ✅ FIX APPLIED HERE: Create timedelta columns after aligning time ---
            df1_aligned['time_hms'] = pd.to_timedelta(df1_aligned['time'], unit='s')
            df2_aligned['time_hms'] = pd.to_timedelta(df2_aligned['time'], unit='s')
            # --- END FIX ---
